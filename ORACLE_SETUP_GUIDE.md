# BINANA Oracle A1 deployment guide â€” Hardening V2

Status: **source-reviewed implementation; Oracle validation pending**. This is the LIVE-capable package, but every procedure below keeps it in `EXECUTION_MODE=simulation` with `BINANCE_TESTNET=true`. Git publication alone does not mean it has been deployed, connected to real credentials, promoted, or validated on Oracle. Completing these steps proves host deployment only; it does not prove profitability, authenticated TestNet order handling, Oracle soak, or LIVE safety.
...
Copy values from `/etc/binana-freqtrade-v101/.env.template`. Preserve these LIVE-package, simulation-safe identities:
...
The installer never sources or evaluates this file. A strict parser validates root ownership, `0600`, non-symlink identity, parent permissions, duplicate/malformed keys, and treats valuesâ€”including `$()`, backticks, semicolons, pipes, redirections and shell expansionsâ€”as literal bytes.
...
5. Deploy through the narrow wrapper:
   ...
   sudo /usr/local/sbin/binana-deploy \
     /var/lib/binana-deploy/inbox/binance-bot-live-trading-COMMIT_SHA.tar.gz \
     /var/lib/binana-deploy/inbox/binance-bot-live-trading-COMMIT_SHA.tar.gz.sha256
   ```
...
Before accepting even simulation operation of this LIVE-capable package, record evidence for: fresh installation; Docker restart; each-container restart; VM reboot; network and DNS interruption; Binance public-endpoint interruption; Telegram interruption; disk warning/critical pressure; memory pressure/OOM; consistent backup; staged restore; previous-release rollback; artifact corruption; unapproved artifact rejection; malformed/symlink `.env`; occupied monitor port; and time-sync loss. Authenticated TestNet lifecycle evidence belongs to the separate TestNet package and must precede any later LIVE review.
