...
[[ "$PRIVATE" == /etc/binana-live ]] || fail 'PRIVATE identity mismatch'
[[ "$APP_ROOT" == /opt/binana-live ]] || fail 'APP_ROOT identity mismatch'
[[ "$PERSIST" == /var/lib/binana-live/shared ]] || fail 'PERSIST identity mismatch'
[[ "$MONITOR_LOG_DIR" == /var/log/binana-live/monitor ]] || fail 'MONITOR_LOG_DIR identity mismatch'
[[ "$DEPLOY_INBOX" == /var/lib/binana-live/deploy-inbox ]] || fail 'DEPLOY_INBOX identity mismatch'
...
install -m 0755 -o root -g root "$SCRIPT_DIR/docker_firewall.sh" /usr/local/sbin/binana-live-docker-firewall
cat >/etc/systemd/system/binana-live-docker-firewall.service <<'EOF'
[Unit]
...
[Service]
Type=oneshot
ExecStart=/usr/local/sbin/binana-live-docker-firewall
...
systemctl enable --now binana-live-docker-firewall.service
...
install -m 0755 -o root -g root "$SCRIPT_DIR/binana-deploy-wrapper.sh" /usr/local/sbin/binana-live-deploy
install -m 0755 -o root -g root "$SCRIPT_DIR/binana-approve-release.sh" /usr/local/sbin/binana-live-approve-release
install -d -m 0755 -o root -g root /etc/sudoers.d
cat >/etc/sudoers.d/binana-live-deploy <<EOF
$DEPLOY_USER ALL=(root) NOPASSWD: /usr/local/sbin/binana-live-deploy
EOF
chmod 0440 /etc/sudoers.d/binana-live-deploy
visudo -cf /etc/sudoers.d/binana-live-deploy >/dev/null
cat >/etc/default/binana-live-deploy <<EOF
BINANA_DEPLOY_UID=$deploy_uid
EOF
chmod 0644 /etc/default/binana-live-deploy
...
cat >/etc/sysctl.d/60-binana-live-memory.conf <<'EOF'
vm.swappiness=10
EOF
...
cat >/etc/chrony/conf.d/50-oci-binana-live.conf <<'EOF'
...
cat >/etc/apt/apt.conf.d/52binana-live-unattended-upgrades <<'EOF'
...
cat >/etc/systemd/journald.conf.d/60-binana-live.conf <<'EOF'
[Journal]
SystemMaxUse=512M
RuntimeMaxUse=128M
