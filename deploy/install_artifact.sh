...
[[ $EUID -eq 0 ]] || fail 'install_artifact.sh must run through the root-owned binana-live-deploy wrapper'
[[ "$APP_ROOT" == /opt/binana-live ]] || fail 'APP_ROOT identity mismatch'
[[ "$RELEASES" == /opt/binana-live/releases ]] || fail 'RELEASES identity mismatch'
[[ "$CURRENT" == /opt/binana-live/current ]] || fail 'CURRENT identity mismatch'
[[ "$PERSIST" == /var/lib/binana-live/shared ]] || fail 'PERSIST identity mismatch'
[[ "$ENV_FILE" == /etc/binana-live/.env ]] || fail 'ENV_FILE identity mismatch'
...
disable_monitoring_after_failed_first_install(){
  ...
  local units=(
    ...
    ${SYSTEMD_PREFIX}-monitor-report-live.timer
...
[[ "$BOT_INSTANCE_ID" =~ ^BINANA-LIVE-[A-Z0-9][A-Z0-9-]{2,47}$ ]] || fail 'BOT_INSTANCE_ID must use the BINANA-LIVE namespace'
