...
[[ $# -eq 2 ]] || fail 'usage: binana-live-deploy RELEASE.tar.gz RELEASE.tar.gz.sha256'

ARTIFACT=$1
CHECKSUM=$2
INBOX=${BINANA_DEPLOY_INBOX:-/var/lib/binana-live/deploy-inbox}
APPROVED_FILE=${BINANA_APPROVED_RELEASE_FILE:-/etc/binana-live/approved-release.sha256}
DEPLOY_DEFAULTS=/etc/default/binana-live-deploy
[[ "$INBOX" == /var/lib/binana-live/deploy-inbox ]] || fail 'deployment inbox must remain fixed'
[[ "$APPROVED_FILE" == /etc/binana-live/approved-release.sha256 ]] || fail 'approval file must remain fixed'
...
stage=$(mktemp -d /var/tmp/binana-live-deploy.XXXXXX)
