...
3. **deploy-oracle** (only on manual `workflow_dispatch` from `main` when `vars.ORACLE_LIVE_DEPLOY_ENABLED == 'true'`): transfers the verified
   artifact and invokes the root-owned `binana-deploy` wrapper. The wrapper refuses the artifact unless its
   exact archive SHA-256 was independently approved on the host; CI cannot approve its own release or
...
- Configure the `oracle-production` environment secrets: `ORACLE_SSH_PRIVATE_KEY`, `ORACLE_SSH_KNOWN_HOSTS`,
  `ORACLE_HOST`, `ORACLE_USER`, `ORACLE_SSH_PORT`.
- Require reviewers and restrict deployment branches on `oracle-production`.
  A push builds and verifies artifacts but cannot deploy this live package.
...
## Manual rollback
```
sudo ln -sfn /opt/binana-live/releases/<previous-stamp> /opt/binana-live/current
cd "$(readlink -f /opt/binana-live/current)"
sudo env RELEASE_TAG="$(cat .release-tag)" COMPOSE_PROJECT_NAME=binana-live \
  docker compose --env-file /etc/binana-live/.env up -d
