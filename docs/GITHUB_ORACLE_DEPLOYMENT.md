...
Keep private trading material only on Oracle in root-owned `/etc/binana-live/.env` with mode `600`, and runtime/private data under `/var/lib/binana-live/shared`.
...
Configure a protected `oracle-production` GitHub environment with required
reviewers. Set `ORACLE_LIVE_DEPLOY_ENABLED=true` only after every external gate
is documented for the exact artifact hash. Live deployment is manual
`workflow_dispatch`; a push never deploys this package.

## Oracle preparation

```bash
sudo ./deploy/oracle_setup.sh
sudoedit /etc/binana-live/.env
sudo chmod 600 /etc/binana-live/.env
```
...
SHARED_HOST_PATH=/var/lib/binana-live/shared
...
`/var/lib/binana-live/shared` retains:
...
```text
/var/lib/binana-live/shared/runtime/SIDECAR_LIVE_OK
```
...
Keep `EXECUTION_MODE=simulation` until all gates are complete. Run testnet with
the separate testnet package; the live-package installer deliberately rejects
`EXECUTION_MODE=testnet` to prevent cross-mode deployment.
