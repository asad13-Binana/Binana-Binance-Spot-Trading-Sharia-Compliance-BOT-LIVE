...
1. Install the release and put real credentials only in
   `/etc/binana-live/.env` (`root:root`, mode `0600`).
2. Run:
...
4. Read the sanitised status through the authenticated loopback monitoring API
   ...
   `/var/lib/binana-live/shared/runtime/api_readiness_status.json`.
...
Sharia approval, or LIVE-money certification. The LIVE package must remain in
simulation until its separate signed promotion evidence is complete.
