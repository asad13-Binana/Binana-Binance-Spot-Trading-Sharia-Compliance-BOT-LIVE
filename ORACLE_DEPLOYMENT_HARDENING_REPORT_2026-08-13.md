...
| High | The production Oracle `SHARED_HOST_PATH` template caused container CI to mount an empty root-owned directory instead of its checked-out Sharia fixtures. | CI now explicitly overrides only its generated `.env.ci` with `SHARED_HOST_PATH=./shared`; production retains the absolute Oracle path. A regression test pins this separation. |
